...
const User = sequelize.define('user', {
    ...
    role:{type: DataTypes.STRING, defaultValue:'USER', validate:{
        len:{
            args:[3, 10],
    ...
    name:{type: DataTypes.STRING, validate:{len:{args:[3, 30], msg:'Role must contain between 3 and 30 characters.'},    is: {
...
const Basket = sequelize.define('basket', {
    id:{type: DataTypes.INTEGER, primaryKey:true, autoIncrement:true},
    count:{type:DataTypes.INTEGER, validate:{
        myValid:(num)=>{
            if(num<=0){
...
const Device = sequelize.define('device', {
    id:{type: DataTypes.INTEGER, primaryKey:true, autoIncrement:true},
    name:{type: DataTypes.STRING, unique:true, allowNull:false, validate:{
        len:{
            args:[3, 150],
...
const Rating = sequelize.define('rating', {
    id:{type: DataTypes.INTEGER, primaryKey:true, autoIncrement:true},
    rate:{type: DataTypes.DECIMAL, allowNull:false, validate:{
        min:0,
        max:5
    }},
    comment:{ type:DataTypes.STRING, allowNull:true, validate:{
        len:{
            args:[0, 50000], 
...
const DeviceInfo = sequelize.define('device_info', {
    ...
    textPart:{type:DataTypes.STRING, validate:{
        len:{
            args:[0, 300],
